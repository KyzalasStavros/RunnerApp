...
class PaceCalculator:
    ...
    def _render_pace_inputs(self):
        ...
        # Determine what values to show in the widgets
        time_total = st.session_state.pace_time_min + (st.session_state.pace_time_sec / 60.0)
        distance = st.session_state.pace_distance
        ...
        if time_total > 0 and distance > 0:
            ...
            print(f"🎯 WIDGET DEBUG: Using calculated values - {display_pace_min}'{display_pace_sec:02d}''")
        else:
            # Show default values when no calculation
            display_pace_min = 7
            display_pace_sec = 30
            print(f"🎯 WIDGET DEBUG: Using default values - {display_pace_min}'{display_pace_sec:02d}''")
