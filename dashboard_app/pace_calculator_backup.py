...
class PaceCalculator:
    ...
    def render(self):
        ...
        fixed_mode = st.toggle(
            "Fixed Pace Mode", 
            value=False,  # Fixed default value
        ...
        print(f"🔧 TOGGLE DEBUG: Session state = {st.session_state.get('fixed_pace_mode', False)}")
    ...
    def _render_pace_display(self):
        """Render pace display header using current widget values"""
        # Get current widget values directly for immediate display
        ...
        st.markdown(f"**🏃 Pace ({pace_display} min/km)**")
        
        # DEBUG: Show what we're displaying
        print(f"📊 DISPLAY DEBUG: Showing pace as {pace_display} (from widgets: {current_pace_min}'{current_pace_sec:02d}'')")
    ...
    def _render_pace_inputs(self):
        ...
        # Always use the current widget values
        self.manual_pace = input_pace_min + (input_pace_sec / 60.0)
        
        # DEBUG: Track pace input changes
        print(f"🎯 INPUT DEBUG: Widget values - min={input_pace_min}, sec={input_pace_sec}, total={self.manual_pace:.2f}")
